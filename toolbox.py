...
# ==========================================
def create_google_sheet_with_gemini(instructions, data_context, user_email):
    import requests
    import csv
    import io
    import streamlit as st
    ...
    # 1. Send the payload to Gemini 1.5 Pro
    gemini_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent?key={gemini_key}"
    
    # We strictly enforce CSV output so we can pipe it directly into Google Sheets
    prompt = f"""You are an expert freight data analyst. Analyze the following data based on these instructions: {instructions}
    
    DATA TO ANALYZE:
    {data_context}
    
    OUTPUT INSTRUCTIONS: Output your final analysis STRICTLY as valid CSV formatted text. Do not include markdown blocks like ```csv. Do not include any conversational text. Only return the raw CSV."""
    ...
    try:
        resp = requests.post(gemini_url, headers={"Content-Type": "application/json"}, json=payload, timeout=45)
        if resp.status_code != 200:
            return f"Gemini API Error: HTTP {resp.status_code} - {resp.text}"
            
        resp_json = resp.json()
        csv_text = resp_json['candidates'][0]['content']['parts'][0]['text']
        
        # Clean up the output in case Gemini includes markdown
...
Once both of these are committed, go to your Streamlit app, upload a document or dataset, and give BOOF the command: *"Use Gemini to analyze this and output the results into a Google Sheet."*
